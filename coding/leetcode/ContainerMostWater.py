...
class Solution(object):
    def maxArea(self, height):
        ...
        :rtype: int
        Two-pointer sliding windwos solution, start from left and right edge.
        Always advance from shorter edge toward the otherside.
        """
        if(min(height)<0):
            raise ValueError("Input must be non negative.")
        N = len(height)
        if(N<2):
            raise ValueError("Length of the input must be at least 2.")
        maxVal = 0
        left, right = 0, N-1
        while left<right:
    ...
    def maxArea1(self, height):
        ...
        :type height: List[int]
        :rtype: int
        Most straightforward solution, scan through all possible rectangles
...
if __name__ == '__main__':
    a = Solution()
    testVector = [[1,2,3,4,6],[5,4,3,7,9,2,1],
            [9,  3, 13,  7, 17, 10,  5,  8,  9, 17],
            [26, 35, 89,  7, 76, 11, 22, 71, 23,  8]]
    for test in testVector:
        print('original={}, Max area is {},{}'.format(
                test, a.maxArea(test),a.maxArea1(test)))
